...
function runGame(plans, Parser, Display) {
  ...
  return new Promise(done => {
    ...
    backgroundMain(0);
    //let n =0;
    //let imagelevel = `url(mbg${n}.jpg)`;
    //     pole.style.backgroundImage = imagelevel;
  });
  function backgroundMain(n) {
    const element = document.getElementById("pole");
    if (n === 0) {
      let background = element.classList.add("background-one");
    }
    else if (n === 1) {
      let background = element.classList.add("background-two");
    }
    else if (n === 3) {
      let background = element.classList.add("background-win");
    }
  }
  function livesMain(myLives) {
    return lives.innerText = `Жизни` + myLives;
  }
}
